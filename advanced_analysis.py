...
ax1.set_xlabel('Drift Velocity (m/s)', fontsize=12)
ax1.set_ylabel('Data Rate (a.u.)', fontsize=12)
ax1.set_title('Data Rate vs Drift Velocity', fontsize=13, fontweight='bold')
...
ax2.set_xlabel('Drift Velocity (m/s)', fontsize=12)
ax2.set_ylabel('Bit Error Rate (BER)', fontsize=12)
ax2.set_title('BER vs Drift Velocity', fontsize=13, fontweight='bold')
...
for res in length_results:
    axes[0].plot(t, res['h_t'], linewidth=2, label=f"L={res['length']*1000:.1f}mm")
axes[0].set_xlabel('Time (s)', fontsize=12)
axes[0].set_ylabel('Impulse Response h(t)', fontsize=12)
axes[0].set_title('Impulse Responses at Different Lengths', fontsize=13, fontweight='bold')
axes[0].legend(fontsize=10)
axes[0].grid(alpha=0.3)
axes[0].set_xlim(0, 0.3)

# Plot 2: Arrival time vs length
lens = [r['length']*1000 for r in length_results]
arrivals = [r['mean_arrival'] for r in length_results]
axes[1].plot(lens, arrivals, 'go-', linewidth=2.5, markersize=8)
axes[1].set_xlabel('Channel Length (mm)', fontsize=12)
axes[1].set_ylabel('Mean Arrival Time (s)', fontsize=12)
axes[1].set_title('Arrival Time vs Channel Length', fontsize=13, fontweight='bold')
axes[1].grid(alpha=0.3)

# Plot 3: Data rate vs length
rates = [r['data_rate'] for r in length_results]
axes[2].plot(lens, rates, 'mo-', linewidth=2.5, markersize=8)
axes[2].set_xlabel('Channel Length (mm)', fontsize=12)
axes[2].set_ylabel('Data Rate (a.u.)', fontsize=12)
axes[2].set_title('Data Rate vs Channel Length', fontsize=13, fontweight='bold')
...
axes[0].plot(t, diffusion_only['h_t'], 'b-', linewidth=2.5, label='Diffusion Only')
axes[0].plot(t, drift_assisted['h_t'], 'r-', linewidth=2.5, label='Drift-Assisted')
axes[0].set_xlabel('Time (s)', fontsize=12)
axes[0].set_ylabel('Impulse Response h(t)', fontsize=12)
axes[0].set_title('Final Comparison: Channel Impulse Response', fontsize=13, fontweight='bold')
axes[0].legend(fontsize=11)
axes[0].grid(alpha=0.3)

cum_diff = np.cumsum(diffusion_only['h_t'])
cum_drift = np.cumsum(drift_assisted['h_t'])
axes[1].plot(t, cum_diff, 'b-', linewidth=2.5, label='Diffusion Only')
axes[1].plot(t, cum_drift, 'r-', linewidth=2.5, label='Drift-Assisted')
axes[1].set_xlabel('Time (s)', fontsize=12)
axes[1].set_ylabel('Cumulative Response', fontsize=12)
axes[1].set_title('Channel Memory Comparison', fontsize=13, fontweight='bold')
